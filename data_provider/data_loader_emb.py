...
class Dataset_ETT_hour(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Load embeddings with shape [768, N, P]
        file_path = os.path.join(self.embed_path, f"{index}.h5")

        if os.path.exists(file_path):
            with h5py.File(file_path, 'r') as hf:
                data = hf['embeddings'][:]  # Expected shape: [768, N, P] or [768, 7, 4]
                embeddings = torch.from_numpy(data)  # [768, N, P]
        else:
...
class Dataset_ETT_minute(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if os.path.exists(file_path):
            with h5py.File(file_path, 'r') as hf:
                data = hf['embeddings'][:]  # Expected shape: [768, N, P]
                embeddings = torch.from_numpy(data)  # [768, N, P]
        else:
            raise FileNotFoundError(f"No embedding file found at {file_path}")
